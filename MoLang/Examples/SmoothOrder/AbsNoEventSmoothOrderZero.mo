...
model AbsNoEventSmoothOrderZero
  extends Modelica.Icons.Example;
  Modelica.Blocks.Sources.Sine sine(freqHz=10)
    annotation (Placement(transformation(extent={{-42,-10},{-22,10}})));
  Blocks.SmoothOrder.AbsSmoothOrderZero abs1
...
<p><span style=\"font-family: Courier New;\">  sine.y := sine.offset+(if time &lt; sine.startTime then 0 else sine.amplitude*sin</span></p>
<p><span style=\"font-family: Courier New;\">    (6.283185307179586*(sine.freqHz*(time-sine.startTime))+sine.phase));</span></p>
<p><span style=\"font-family: Courier New;\">  der1.u := smooth(0, (if noEvent(sine.y &gt;= 0) then sine.y else  -sine.y));</span></p>
...
<p><span style=\"font-family: Courier New;\">  der(der1.u) := (if noEvent(sine.y &gt;= 0) then der(sine.y) else  -der(sine.y));</span></p>
</html>"));
end AbsNoEventSmoothOrderZero;
