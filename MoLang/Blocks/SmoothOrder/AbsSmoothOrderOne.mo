within MoLang.Blocks.SmoothOrder;
block AbsSmoothOrderOne
  "Output the absolute value of the input with smooth order 1 added which is fake"
  extends Modelica.Blocks.Interfaces.SISO;
  parameter Boolean generateEvent=false
    "Choose whether events shall be generated" annotation (Evaluate=true);
equation
  //y = abs(u);
  y = if generateEvent then (if u >= 0 then u else -u) else smooth(1, (if
    noEvent(u >= 0) then u else -u));
  annotation (
